...
std::unique_ptr<StatementList> Parser::parse() {
    ...
    //     std::cout << int(currentToken.type) << " " << currentToken.value << std::endl;
        
    //     if (currentToken.value == "declare") statements.push_back(parseDeclaration());
    //     else if (currentToken.type == TokenType::Identifier) statements.push_back(parseAssignment());
    //     else if (currentToken.value == "print") statements.push_back(parsePrintStatement());
    //     else if (currentToken.value == "if") statements.push_back(parseIfStatement());
    //     else throw std::runtime_error("Неизвестный оператор");
    // }
